...
main
    ...
    │   │   ├── Glycine_max_a2.v1.fasta.to.GCF_000004515.6_Glycine_max_v4.0_genomic.unmasked.fna.over.chain                 
    ...
    ├──SQL/
    │   ├── SQL.ipynb                   # Ноутбук, где описано взаимодействие с БД (подключение и загрузка таблиц)
    │   ├── soy.tsv                     # Загруженная в бд соя
    │   ├── pea.tsv                     # Загруженный в бд горох
    │   ├── rice.tsv                    # Загруженный в бд рис
    │   ├── sunflower.tsv               # Загруженный в бд подсолнечник
    ├──READMY.md
```
...
1. **Чтение входного файла**:   
    ...
    - pos_start_original — начальная позиция региона в исходной сборке      
    - pos_end_original — конечная позиция региона в исходной сборке      
2. **Создание BED файла**:         
    Скрипт извлекает данные из столбцов chr_original, pos_start_original и pos_end_original входного файла и создает на их основе новый файл в формате <file_name>.bed. Этот файл будет содержать:
    - chr_original — название хромосомы в исходной сборке      
    - pos_start_original — начальная позиция региона в исходной сборке      
    - pos_end_original — конечная позиция региона в исходной сборке      
...
3. **Преобразование координат с использованием chain-файла**:      
    ...
    - chr — название хромосомы в целевой сборке      
    - pos_start — начальная позиция региона в целевой сборке     
    - pos_end — конечная позиция региона в целевой сборке     
4. **Сохранение результатов**:        
    После преобразования координат скрипт создает новый .tsv файл под именем <file_name_assembly>.tsv, где assembly — название целевой сборки. В этот файл записываются все исходные данные, а также новые столбцы с преобразованными координатами (chr, pos_start, pos_end).
...
Скрипт поддерживает любой порядок столбцов, главное, чтобы названия не менялись и обязательно были следующие столбцы: chr_original, pos_start_original, pos_end_original, chr, pos_start, pos_end.

## Сравнение CoordTransfer с SNPLift
![image](https://github.com/user-attachments/assets/898f89e8-4a5f-4d15-ad0c-672940bf3e5d)
Сравнивая перевод 318 позиций, этот скрипт идентично snplift заполнилнил 311 позиций. Оставшиеся 7 позиций: 3 позиции snplift просто не нашел, а этот скрипт смог перенести координаты, для еще 4 позиций отличие было в 1 нуклеотид (см. сравнение с snplift в папке Excel: https://github.com/erofeeva-plastilin/Converting_Genome_Coordinates_pipeline/tree/main/Excel). Скорость выполнения скрипта для этого составила **0m1.657s**, что значительно быстрее snplift.
Для расходившихся 4 позиций (на исходной сборке:Gm13:7762318, Gm11:38372080, Gm16:19302037, Gm16:19474288), была проведена проверка, в ходе которой были извлечены и сопоставлены эти последовательности из исходной и целевой сборок. 
```
# Последовательности для исходных координат
samtools faidx Glycine_max_a1.v1.fasta Gm13:7762318-7762318 # A
samtools faidx Glycine_max_a1.v1.fasta Gm11:38372080-38372080 # T
samtools faidx Glycine_max_a1.v1.fasta Gm16:19302037-19302037 # G
samtools faidx Glycine_max_a1.v1.fasta Gm16:19474288-19474288 # C
# Последовательности для переведенных координат
## SNPLift
samtools faidx GCF_000004515.6_Glycine_max_v4.0_genomic.unmasked.fna NC_038249.2:13151458-13151458 # T
samtools faidx GCF_000004515.6_Glycine_max_v4.0_genomic.unmasked.fna NC_038247.2:39037904-39037904 # A
samtools faidx GCF_000004515.6_Glycine_max_v4.0_genomic.unmasked.fna NC_038252.2:21209491-21209491 # C
samtools faidx GCF_000004515.6_Glycine_max_v4.0_genomic.unmasked.fna NC_038252.2:21035550-21035550 # G
## CoordTransfer
samtools faidx GCF_000004515.6_Glycine_max_v4.0_genomic.unmasked.fna NC_038249.2:13151457-13151457 # C
samtools faidx GCF_000004515.6_Glycine_max_v4.0_genomic.unmasked.fna NC_038247.2:39037903-39037903 # T
samtools faidx GCF_000004515.6_Glycine_max_v4.0_genomic.unmasked.fna NC_038252.2:21209490-21209490 # T
samtools faidx GCF_000004515.6_Glycine_max_v4.0_genomic.unmasked.fna NC_038252.2:21035549-21035549 # T
```
CoordTransfer успешно перевел координаты для одной из позиций (Gm11:38372080), где последовательность в целевой сборке совпала с исходной. В то же время snplift не смог корректно перевести ни одну из 4 позиций.

CoordTransfer использует chain-файл, показывающий соответствия между сборками. Этот файл создается инструментом на основе UCSC LiftOver — надежного и проверенного метода. Однако, если chain-файл содержит разрывы, ошибки или неоднозначности, это может привести к незначительным смещениям координат, особенно на границах регионов. Расхождения в 1 нуклеотид также могут быть вызваны округлением или разной интерпретацией координат.

Дополнительно, геномные сборки, используемые для анализа, относятся к разным годам. Это может объяснять часть расхождений естественными биологическими вариациями или техническими особенностями сборок. Такие отличия могут отражать как различия в исходных данных, так и ограничения используемых инструментов.

Таким образом, CoordTransfer показал более высокое качество работы, несмотря на некоторые расхождения, что свидетельствует о большей точности его алгоритма в интерпретации данных из chain-файла.

Проверка другой сборки сои:
![image](https://github.com/user-attachments/assets/448cd8e0-caf4-47ec-b2de-426366af0a5e)
